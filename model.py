...
def get_model(point_cloud,is_training, bn_decay=None,weight_decay=None):
  """ ConvNet baseline, input is BxNx9 gray image """
  """EDGCNet based on DGCNN, hyperbolic transformation is placed in tf_util"""
  ...
  top_feature, nn_idx = tf_util.knn(adj, k=k) # (batch, num_points, k)
  edge_feature = tf_util.get_edge_feature(input_image, nn_idx=nn_idx, k=k)
  T_E_H_feature = tf_util.get_E_H_feature(top_feature,nn_idx,bn_decay,is_training,weight_decay,scope='E_H_1',mlp=24)
  edge_feature = tf.concat([edge_feature,T_E_H_feature],axis=-1)
  ...
  top_feature,nn_idx = tf_util.knn(adj, k=k)
  edge_feature = tf_util.get_edge_feature(net_1, nn_idx=nn_idx, k=k)
  
  T_E_H_feature = tf_util.get_E_H_feature(top_feature,nn_idx,bn_decay,is_training,weight_decay,scope='E_H_2',mlp=24)
  ...
  top_feature,nn_idx = tf_util.knn(adj, k=k)
  T_E_H_feature = tf_util.get_E_H_feature(top_feature,nn_idx,bn_decay,is_training,weight_decay,scope='E_H_3',mlp=24)
